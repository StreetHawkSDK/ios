...
class ChannelShareViewController: StreetHawkBaseTableViewController, UITextFieldDelegate
{
    ...
    @IBAction func buttonShareClicked(_ sender: AnyObject)
    {
        ...
        SHApp.sharedInstance().originateShare(withCampaign: self.textboxID.text, withMedium: self.textboxMedium.text, withContent: self.textboxContent.text, withTerm: self.textboxTerm.text, share: deeplinkingUrl, withDefaultUrl: destinationUrl, withMessage: self.textboxMessage.text)
    }
}
