...
@interface InstallCasesViewController ()
...
//when this view open, not read whole file content from install log, but append last row to be efficient.
- (void)installLogsUpdated:(NSNotification *)notification;
//format install data to fill table cell data and refresh table to display.
- (void)displayInstallData;

@end

@implementation InstallCasesViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])
    {
        ...
        [arrayGeneralDescription addObject:@"Your unique identifier for this Client. Tagged by API `[StreetHawk tagString:<unique_value> forKey:@\"sh_cuid\"];`"];
        ...
        [arrayDeviceDescription addObject:@"Descriptive text for the device model, e.g. `iPhone 6`."];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(installLogsUpdated:) name:LogMonitorUpdatedNotification object:nil];
    }
    return self;
}
...
- (void)viewDidUnload
{
    [self setTableViewInstall:nil];
    [self setButtonUpdateInstall:nil];
...
- (void)dealloc
{
    ...
    self.tableViewInstall.dataSource = nil;
    self.tableViewInstall.delegate = nil;
}
...
{
    NSError *error;
    [[InstallServiceMonitor shared] clearLogHistory:&error];
    if (error)
    {
...
- (void)displayInstallData
{
    ...
    {
        //show data to control
        if (StreetHawk.currentInstall == nil)
        {
            [self.buttonUpdateInstall setTitle:@"Register Install" forState:UIControlStateNormal];
        }
        else
        {
            [self.buttonUpdateInstall setTitle:@"Update Install" forState:UIControlStateNormal];
        }
        ...
        arrayGeneralValue[6] = [NSString stringWithFormat:@"AppStore or Enterprise release: %@", shBoolToString(StreetHawk.currentInstall.live)];
        arrayGeneralValue[7] = [NSString stringWithFormat:@"development platform: %@", StreetHawk.currentInstall.developmentPlatform];
        arrayGeneralValue[8] = [NSString stringWithFormat:@"created date: %@", shFormatStreetHawkDate(StreetHawk.currentInstall.created)];
        arrayGeneralValue[9] = [NSString stringWithFormat:@"modifid date: %@", shFormatStreetHawkDate(StreetHawk.currentInstall.modified)];
        arrayGeneralValue[10] = [NSString stringWithFormat:@"replaced by: %@", StreetHawk.currentInstall.replaced];
        arrayGeneralValue[11] = [NSString stringWithFormat:@"uninstalled date: %@", shFormatStreetHawkDate(StreetHawk.currentInstall.uninstalled)];
        NSMutableArray *arrayCapabilityValue = self.arrayValues[1];
        arrayCapabilityValue[0] = [NSString stringWithFormat:@"use location feature: %@", shBoolToString(StreetHawk.currentInstall.featureLocation)];
        arrayCapabilityValue[1] = [NSString stringWithFormat:@"use push feature: %@", shBoolToString(StreetHawk.currentInstall.featurePush)];
        arrayCapabilityValue[2] = [NSString stringWithFormat:@"use iBeacon feature: %@", shBoolToString(StreetHawk.currentInstall.featureiBeacons)];
        arrayCapabilityValue[3] = [NSString stringWithFormat:@"support iBeacon: %@", shBoolToString(StreetHawk.currentInstall.supportiBeacons)];
        ...
        arrayPushValue[4] = [NSString stringWithFormat:@"use smart push: %@", shBoolToString(StreetHawk.currentInstall.smart)];
        ...
        [self.tableViewInstall reloadData];
        [BaseLogMonitor showLogToTextView:self.viewLogs fromMonitor:[InstallServiceMonitor shared]];
    });
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGSize constrainSize = CGSizeMake(self.tableViewInstall.bounds.size.width - 10, 100);
