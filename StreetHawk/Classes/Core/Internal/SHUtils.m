...
NSMutableString *shAppendParamsArrayToString(NSMutableString *str, NSArray *params, BOOL isForPost)
{
    if (params)
    {
        ...
        for (int i = 0; i < params.count; i ++)
        {
            ...
            if (!isForPost)
            {
                sep = [str rangeOfString:@"?"].location == NSNotFound ? @"?" : @"&";
            }
            else
            {
                sep = (str != nil && str.length > 0) ? @"&" : @"";
            }
            [str appendFormat:@"%@%@=", sep, shUrlEncodeFull(NONULL(param))];
            i++; //move to value
            ...
            if (i < params.count)
            {
                ...
                if ([value isKindOfClass:[NSString class]])
                    [str appendString:shUrlEncodeFull(NONULL((NSString *)value))];
                else if ([value isKindOfClass:[NSNumber class]])
                    [str appendString:[((NSNumber *)value) stringValue]];
                else if ([value isKindOfClass:[NSDate class]])
                    [str appendString:shUrlEncodeFull(shFormatStreetHawkDate((NSDate *)value))];
                else
...
NSMutableString *shAppendParamsDictToString(NSMutableString *str, NSDictionary *params, BOOL isForPost)
{
    __block int index = 0;
    [params enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop)
     {
         ...
         if ([key isKindOfClass:[NSString class]])
             [str appendString:shUrlEncodeFull(NONULL((NSString *)key))];
         else if ([key isKindOfClass:[NSNumber class]])
             [str appendString:[((NSNumber *)key) stringValue]];
         else if ([key isKindOfClass:[NSDate class]])
             [str appendString:shUrlEncodeFull(shFormatStreetHawkDate((NSDate *)key))];
         else
             assert(NO && "Only numbers or strings allowed in URL Parameters");
         [str appendString:@"="];
         if (obj && obj != [NSNull null])
         {
             if ([obj isKindOfClass:[NSString class]])
                 [str appendString:shUrlEncodeFull(NONULL((NSString *)obj))];
             else if ([obj isKindOfClass:[NSNumber class]])
                 [str appendString:[((NSNumber *)obj) stringValue]];
             else if ([obj isKindOfClass:[NSDate class]])
                 [str appendString:shUrlEncodeFull(shFormatStreetHawkDate((NSDate *)obj))];
             else
                 assert(NO && "Only numbers or strings allowed in URL Parameters");
         }
         index++;
