...
//  SHKeyChainStore
//
//  Created by Ethan@Streethawk on 04/19/18.
//  Copyright (c) 2018 Streethawk. All rights reserved.
//

#import "SHKeyChainStore.h"

NSString * const SHKeyChainStoreErrorDomain = @"com.streethawk.SHKeyChainStore";
static NSString *_defaultService;

@interface SHKeyChainStore ()

@end

@implementation SHKeyChainStore
...
+ (SHKeyChainStore *)keyChainStore
{
    return [[self alloc] initWithService:nil accessGroup:nil];
}

+ (SHKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (SHKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
+ (SHKeyChainStore *)keyChainStoreWithServer:(NSURL *)server protocolType:(SHKeyChainStoreProtocolType)protocolType
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:SHKeyChainStoreAuthenticationTypeDefault];
}

+ (SHKeyChainStore *)keyChainStoreWithServer:(NSURL *)server protocolType:(SHKeyChainStoreProtocolType)protocolType authenticationType:(SHKeyChainStoreAuthenticationType)authenticationType
{
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = SHKeyChainStoreItemClassGenericPassword;
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(SHKeyChainStoreProtocolType)protocolType
{
    return [self initWithServer:server protocolType:protocolType authenticationType:SHKeyChainStoreAuthenticationTypeDefault];
}

- (instancetype)initWithServer:(NSURL *)server protocolType:(SHKeyChainStoreProtocolType)protocolType authenticationType:(SHKeyChainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = SHKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = SHKeyChainStoreAccessibilityAfterFirstUnlock;
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    SHKeyChainStore *keychain = [SHKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray UIC_KEY_TYPE *)allKeysWithItemClass:(SHKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == SHKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == SHKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(SHKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == SHKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (void)setAccessibility:(SHKeyChainStoreAccessibility)accessibility authenticationPolicy:(SHKeyChainStoreAuthenticationPolicy)authenticationPolicy
{
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == SHKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'SHKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'SHKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case SHKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case SHKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case SHKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case SHKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case SHKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case SHKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case SHKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case SHKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case SHKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case SHKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case SHKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case SHKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case SHKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case SHKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case SHKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case SHKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case SHKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case SHKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case SHKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case SHKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case SHKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case SHKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case SHKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case SHKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case SHKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case SHKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case SHKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case SHKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case SHKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case SHKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case SHKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case SHKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case SHKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case SHKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case SHKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case SHKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case SHKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case SHKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case SHKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case SHKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case SHKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case SHKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case SHKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case SHKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case SHKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case SHKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:SHKeyChainStoreErrorDomain code:SHKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:SHKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:SHKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:SHKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation SHKeyChainStore (Deprecation)
