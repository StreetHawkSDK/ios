...
+ (void)submitInteractivePairs
{
    if (!shStrIsEmpty(StreetHawk.currentInstall.suid))
    {
        ...
        {
            [[SHHTTPSessionManager sharedInstance] POST:@"/apps/submit_interactive_button/" hostVersion:SHHostVersion_V2 body:@{@"button": dictButtons} success:nil failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nullable error)
             {
